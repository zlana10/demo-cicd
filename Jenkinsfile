	 ...
	 stages {
        ...
        stage('Deploy MySQL to DEV') {
            steps {
                ...
                sh 'sleep 20'
                def IP_ADDRESS = sh 'docker inspect -f {{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}} demo-mysql'
                sh 'docker exec -i demo-mysql mysql -h ${IP_ADDRESS} -P 3306 --user=root --password=${MYSQL_ROOT_LOGIN_PSW} < mysql-script'
            }
        }
