	 ...
	 stages {
        ...
        stage('Deploy MySQL to DEV') {
            steps {
                ...
                sh 'sleep 20'
                echo sh 'docker inspect -f {{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}} demo-mysql'
                sh 'docker exec -i demo-mysql mysql -h 172.19.0.2 -P 3306 --user=root --password=${MYSQL_ROOT_LOGIN_PSW} < mysql-script'
            }
        }
